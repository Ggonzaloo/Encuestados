  ...
  configuracionDeBotones: function(){
    ...
    e.botonAgregarPregunta.click(function() {
      ...
      $('[name="option[]"]').each(function() {
        // completar
        if ($.trim($(this).val())) respuestas.push({textoRespuesta: $(this).val(), cantidad: 0})
      })
    ...
    e.botonBorrarPregunta.on('click', function() {
    ...
    e.botonEditarPregunta.on('click', function() {
      const id = parseInt($('.list-group-item.active').attr('id'))
      const nombreAnterior = $('.list-group-item.active').children('h5').text()
